    ...
    def __init__(self):  
        ...
        while not rospy.is_shutdown():
            ...
            if self.image_received:
                ...
                #cv2.imshow("ggs", img)
                #print(" ")
                #print(str(no_class_prediction) + " "+ str(prediction) + " " + str(scores))
                ...
                if (np.array_equal(scores, self.past_scores_all)): pass
                    ...
                    if (self.counter != self.past_ids_last_ten.shape[1]):
                    ...
                    #if (self.counter != 10):
                        #if (no_class_prediction == self.past_id_class):
                            #self.counter += 1
                            #print(self.counter)
                    #else:
                        #self.flag = "ready"
                        #data = no_class_prediction
                    
                
                
                ## Case 1 - Final
                """
                if (scores[0] == self.past_scores_all): pass    # scores repetidos
                else:   # scores NO repetidos
                    
                    if (self.counter != 10):
                        self.data_ids       = np.append(self.data, id_class_prediction)
                        self.data_score_id  = np.append(self.data_score_id, prediction)
                        self.counter += 1
                    else:
                        unique, counts = np.unique(self.data_ids, return_counts=True)
                        for i in counts:
                            if i >= 7:
                                s
                        if ()
                ...
                if (self.flag == "ready"):
                    print(" ")
                    print("id" + str(data) + " " + str(prediction))
                    self.counter = 0
                    self.data_ids = []
                    self.data_ids = np.array(self.data_ids)
                    self.data_score_id = []
