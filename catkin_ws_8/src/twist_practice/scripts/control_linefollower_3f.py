...
class ColorFollowerClass():  
    ...
    def __init__(self):  
        ...
        while not rospy.is_shutdown():
            ...
            v = 0.08                # v control          --- 0.1
            w = xm - self.center_x  # w control
            self.error_act = w
            
            Kv, Kw = 1.0, 0.00040    # Constantes para control P propuestas -- 1.0, 0.0006
            Kd = 0.00001
            
            self.f_v = Kv * v       # Control P para velocidad lineal
            self.f_w = Kw * self.error_act + Kd*((self.error_act - self.error_ant)/(1/freq))       # Control P para velocidad angular
            
            self.error_ant = self.error_act
