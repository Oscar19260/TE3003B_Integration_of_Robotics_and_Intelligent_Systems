        ...
        while not rospy.is_shutdown():
            ...
            if self.image_received:
                ...
                dims = blur_gi.shape
                    ...
                    # Apply canny
                img_canny = cv2.Canny(blur_gi, 0, 60)       # 0, 60 -- 68, 46
                ...
                ba = abs(np.sum(cp_img2, axis=0))
                        ...
                        # Apply 1st and 2nd diff to sum
                diff_1 = np.diff(cp_img2)
                print("ciao")
                diff_2 = np.diff(cp_img2, n = 2)
                print(diff_2.shape)
                diff_2 = np.append(diff_2, [0])
                print(diff_2.shape)
                
                
                    # Find indexes of the maximum N values
                N = 15  # N maximum values
                result1 = np.argpartition(diff_1, -N)[-N:]
                result2 = np.argpartition(diff_2, -N)[-N:]

                idx_r1 = np.sort(result1, axis=None)   # Sort it
                idx_r2 = np.sort(result2, axis=None)   # Sort it
                ...
                res_diffs   = np.where(diffs == np.amax(diffs))
                ...
                for i in m_4[0,:]:
                    cv2.circle(cp_img_2, (m_4[i], int(cp_img.shape[0]/2)), 5, (255,255,255), -1)
                
                cp_img_f = self.bridge.cv2_to_imgmsg(cp_img_2, "mono8")
                
                center_final = Int32()
                #center_final.data = m_2         # Test point 2
