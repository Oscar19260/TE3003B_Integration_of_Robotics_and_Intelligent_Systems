...
class AvoidObstacleClass():  
    ...
    def __init__(self):  
        ...
        self.ao_vel_pub = rospy.Publisher("ao_vel", Twist, queue_size=1) 
        ...
        while not rospy.is_shutdown():  
            ...
            if self.laser_received: 
                ...
                if np.isinf(min(self.lidar_msg.ranges)): #If there are no obstacles 
                    ...
                    vel_msg.linear.x, vel_msg.angular.z = self.control_speed(closest_range, theta_ao)

            self.ao_vel_pub.publish(vel_msg) 
